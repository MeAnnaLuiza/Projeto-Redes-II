...
class GrowingNeuralGas:
    ...
    def fit_network(self, e_b, e_n, a_max, l, a, d, passes=1, plot_evolution=False):
        ...
        for p in range(passes):
            ...
            fim = time.time()
            execucao = fim - inicio
            tempo = str(timedelta(seconds=round(execucao)))
            tempo_execucao = tempo.split(':')
            print('   Pass #%d' % (p + 1) + '- DONE in ~' + tempo_execucao[0] + 'h:'+ tempo_execucao[1] + 'm:' + tempo_execucao[2] + 's\n')

        tempo_fim = time.time()
        tempo_total = tempo_fim - tempo_inicio
        total = str(timedelta(seconds=round(tempo_total)))
        tempo_execucao_total = total.split(':')
        print('Total execution time in ~' + tempo_execucao_total[0] + 'h:'+ tempo_execucao_total[1] + 'm:' + tempo_execucao_total[2] + 's.\n')
    ...
    def plot_clusters(self, clustered_data):
        ...
        color = cmap.colors[:number_of_clusters]
